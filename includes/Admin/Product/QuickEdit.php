...
use WCSM\Admin\Settings\Options; // use the global display-name formatter
...
class QuickEdit
{
	...
	{
		$options = self::get_supplier_options(false);
	...
	public static function save_quick_edit($product_or_id): void
	{
		...
		// 2) Save supplier price only for simple products with a supplier selected
		if (null !== $price_raw) {
			$supplier_id = (int) get_post_meta($post_id, self::META_SUPPLIER_ID, true);
			if ('simple' === $product->get_type() && $supplier_id > 0) {
				...
				if ('' === $val) {
					delete_post_meta($post_id, self::META_SUPPLIER_PRICE); // fallback to regular price
				} else {
	 ...
	 * - Ensure price input is disabled when hidden so stale values aren't submitted
	 */
	...
	public static function render_bulk_edit_fields($column_name, $post_type): void
	{
		...
		$options = self::get_supplier_options(true);
		?>
	...
	/**
	 * Build supplier <option> list using the global display format.
	 *
	 * @param bool $for_bulk When true, include the "-1 No change" sentinel.
	 * @return array value => label
	 */
	private static function get_supplier_options( bool $for_bulk = false ) : array {
		$options = [];

		if ( $for_bulk ) {
			$options['-1'] = __('— No change —', 'wc-supplier-manager');
		}
		...
		$users = get_users([
			'role'    => 'supplier',
			'orderby' => 'display_name', // DB order; labels will still reflect the chosen format
			'order'   => 'ASC',
			// fetch full objects so the formatter can access names/email/company
		]);

		foreach ( $users as $user ) {
			$options[ (string) $user->ID ] = Options::format_supplier_name( $user );
		}
