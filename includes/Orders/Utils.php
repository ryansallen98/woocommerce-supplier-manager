...
class Utils
{
	...
	{
		$current = (string) $order->get_meta(self::META_ORDER_SUPPLIERS, true);

		// Build the *expected* value from actual line items right now
		...
		if ($current !== $expected) {
			$order->update_meta_data(self::META_ORDER_SUPPLIERS, $expected);
			$order->save();
			return $expected;
		}
